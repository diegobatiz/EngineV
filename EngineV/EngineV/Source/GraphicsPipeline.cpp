...
GraphicsPipeline::GraphicsPipeline(const Renderer& renderer)
{
	mRenderer = &renderer;
}

void GraphicsPipeline::Initialize(const Vertex& vertex, VkExtent2D swapChainExtent, VkDescriptorSetLayout layout, VkRenderPass renderPass)
{
	...
	if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &mLayout) != VK_SUCCESS)
	{
